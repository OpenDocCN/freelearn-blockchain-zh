...
**量子图像处理** (**QIP**) 丰富了现有图像处理技术的能力。使用经典信息处理技术需要大量的计算资源。当使用 QIP 时，存储* N *比特信息发生在* log(2N) *量子比特中。听起来有希望的两种图像处理方法是**量子傅里叶变换** （**QFT**） 和** 量子小波变换** （** QVT **）。使用量子比特来存储图像信息的附加优势是可以利用叠加等属性。
